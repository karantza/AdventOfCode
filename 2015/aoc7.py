...
def valueOf(x):
	if type(x) is str:
		try:
			y = int(x) # this is a string literal
			return y
		except ValueError:
			y = valueOf(variables[x])
			variables[x] = y
			return y
	elif type(x) is int:
		return x
		...
		v = x.eval()
		print(x.s.rstrip() + " == " + str(c_uint16(v).value))
		return valueOf(v)
...
class NodeAnd(Node):
	def eval(self):
		return valueOf(self.a) & valueOf(self.b)

class NodeOr(Node):
	def eval(self):
		return valueOf(self.a) | valueOf(self.b)

class NodeLShift(Node):
	def eval(self):
		return valueOf(self.a) << valueOf(self.b)

class NodeRShift(Node):
	def eval(self):
		return valueOf(self.a) >> valueOf(self.b)


r_ass = re.compile(r'(\w+) -> (\w+)')
r_not = re.compile(r'NOT (\w+) -> (\w+)')
r_bin = re.compile(r'(\w+) ([A-Z]+) (\w+) -> (\w+)')
...
def parse(s):
	...
	if isAss:
		...
		n.s = s
		n.a = isAss.group(1)
		variables[isAss.group(2)] = n

	if isNot:
		n = NodeNot()
		n.s = s
		n.a = isNot.group(1)
		variables[isNot.group(2)] = n
